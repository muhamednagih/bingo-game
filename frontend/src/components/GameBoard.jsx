...
export default function GameBoard({ roomState, playerId, onPlayTurn }) {
    ...
    return (
        <div className="flex flex-col md:flex-row gap-6 lg:gap-8 w-full max-w-6xl mx-auto items-start">
            ...
            <div className="flex flex-col flex-1 w-full">
                ...
                <div className={`bg-board border-4 ${me.lines >= roomState.requiredLines - 1 ? 'border-accent shadow-[0_0_30px_rgba(236,72,153,0.5)]' : 'border-gray-800 shadow-2xl'} rounded-2xl p-4 sm:p-6 w-full transition-all duration-500`}>
                    <div
            ...
            <div className="flex flex-col w-full md:w-[250px] lg:w-[300px] flex-shrink-0">
                <h3 className="text-sm uppercase tracking-widest text-gray-400 mb-3 font-semibold md:mt-0 mt-4">Player Progress</h3>
                <div className="flex flex-col gap-4 bg-gray-900/50 backdrop-blur-sm p-4 lg:p-5 rounded-2xl border border-gray-700 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                    {players.map((p, idx) => {
                        const isMe = p.id === playerId;
                        return (
                            <div key={p.id} className={`bg-gray-800 border-2 ${isMe ? 'border-primary/50 shadow-[0_0_10px_rgba(59,130,246,0.2)]' : 'border-gray-700'} rounded-xl px-4 py-3 flex flex-col w-full transition-all duration-300 hover:border-gray-500`}>
                                <div className="flex justify-between items-center mb-2">
                                    <span className="font-semibold truncate text-white max-w-[120px] lg:max-w-[150px]">
                                        {p.name} {isMe && <span className="text-xs text-gray-400 font-normal ml-1">(You)</span>}
                                    </span>
